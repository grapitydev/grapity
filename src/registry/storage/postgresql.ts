...
export class PostgreSQLSpecStore implements SpecStore, GatewayConfigStore {
  ...
  async getLatestGatewayConfigVersion(name: string): Promise<GatewayConfigVersion | null> {
    ...
    const rows = await this.db.select().from(gatewayConfigVersions)
      ...
      .orderBy(desc(gatewayConfigVersions.createdAt), desc(gatewayConfigVersions.id))
      .limit(1);
    if (rows.length === 0) return null;
    return this.mapGatewayConfigVersionRow(rows[0]);
  }
  ...
  async listGatewayLogs(filters: GatewayLogFilters): Promise<{ logs: GatewayLog[]; total: number }> {
    ...
    if (filters.from) {
      conditions.push(sql`${httpLogs.occurredAt} >= ${filters.from}`);
    }
    if (filters.to) {
      conditions.push(sql`${httpLogs.occurredAt} <= ${filters.to}`);
    }
  ...
  async getGatewayLogStats(_filters: GatewayLogFilters): Promise<GatewayLogStats[]> {
    ...
    if (_filters.from) {
      conditions.push(sql`${httpLogs.occurredAt} >= ${_filters.from}`);
    }
    if (_filters.to) {
      conditions.push(sql`${httpLogs.occurredAt} <= ${_filters.to}`);
    }
  ...
  async deleteGatewayLogsOlderThan(days: number): Promise<void> {
    ...
    await this.db.delete(httpLogs).where(sql`${httpLogs.occurredAt} < ${cutoff}`);
  }
}
